  ...
  try {
    ...
    if (!verifyAuthToken(authToken)) {
      return NextResponse.json(
        { error: "Unauthorized: Admin password required to upload diagrams" },
        { status: 401 },
      );
    }
    ...
    const cleanPaper = pyq.paper
      .replace(/[^a-zA-Z0-9]/g, "_")
      .replace(/_+/g, "_")
      .replace(/^_|_$/g, "")
      .toLowerCase();
    const cleanStage = pyq.examStage
      .replace(/[^a-zA-Z0-9]/g, "_")
      .replace(/_+/g, "_")
      .replace(/^_|_$/g, "")
      .toLowerCase();
    ...
    // 1. Upload directly to Azure Blob Storage (Student Pack Cloud Storage)
    let publicUrl = "";
    try {
      ...
      if (azureUrl) {
        publicUrl = azureUrl;
        console.log(
          "Successfully uploaded diagram to Azure Blob Storage:",
          azureUrl,
        );
      } else {
      ...
      return NextResponse.json(
        { error: "Failed to upload image to Azure Storage" },
        { status: 500 },
      );
    }
